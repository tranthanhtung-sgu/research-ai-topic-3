...
class CinC2020(Metric):
    ...
    def compute(self):
        ...
        assert len(gt) == len(preds), f"gt and preds must have the same shape. Found {gt.shape} and {preds.shape}"
        assert gt.size(1) == len(self.conditions), f"Number of conditions in gt and preds must be the same. Found {gt.shape[1]} and {len(self.conditions)}"
        if self.verbose:
            ...
            print()
        normal_class = 'NORM'
        # create dataframes for gt and preds. Columns are the conditions
        ...
        assert gt.shape[1] == len(self.conditions), f"Number of conditions in gt and preds cols must be the same. Found {gt.shape[1]} and {len(self.conditions)}"
        assert gt.shape == preds.shape, f"gt and preds must have the same shape. Found {gt.shape} and {preds.shape}"
        ...
        assert task_weights.index.tolist() == self.conditions, f"task_weights index must be the same as conditions. Found {task_weights.index.tolist()} and {self.conditions}"
        assert task_weights.columns.tolist() == self.conditions, f"task_weights columns must be the same as conditions. Found {task_weights.columns.tolist()} and {self.conditions}"
        task_weights = task_weights.to_numpy(dtype=float)
        assert task_weights.shape == (len(self.conditions), len(self.conditions)), f"task_weights must have shape len(conditions) x len(conditions). Found {task_weights.shape}"
        ...
        if normal_index is not None:
            ...
            inactive_score = 0.0
        if self.verbose:
            logger.info(f"Observed score: {observed_score}")
            logger.info(f"Correct score: {correct_score}")
            logger.info(f"Inactive score: {inactive_score}")
