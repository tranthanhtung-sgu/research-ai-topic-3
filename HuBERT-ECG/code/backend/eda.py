...
def plot_rr_interval_distribution_comparative(
    ...
    # Instead of saving the figure, return it
    if not fig.get_size_inches()[0] > 1 or not fig.get_size_inches()[1] > 1:
        fig.set_size_inches(12, 14)  # Larger default for comparative
    try:
        fig.tight_layout(rect=[0, 0.03, 1, 0.95])
    except Exception:
        pass
...
def plot_ecg_segments_comparative(
    ...
    # Instead of saving the figure, return it
    try:
        fig_superimposed.tight_layout(rect=[0, 0.03, 1, 0.95])
    except Exception:
        pass
        
    return fig_superimposed
...
def make_figs(ecg_data, case_condition_short_label, ref_npy_path_relative):
    # Handle both file paths and direct numpy arrays
    case_npy_file_path = ecg_data  # This might be a path or a numpy array
    case_condition_label_short = case_condition_short_label  # Use provided label
    case_condition_label_full = f"{case_condition_short_label} CASE"  # Format full label
    case_sampling_rate = 100  # Keep the sampling rate as 100Hz
    case_preferred_lead_idx = 1  # Keep Lead II as default
    
    # Reference path should use the provided relative path
    normal_ref_npy_file_path = ref_npy_path_relative
    ...
    try:
        # Check if ecg_data is already a numpy array
        if isinstance(ecg_data, np.ndarray):
            ecg_all_leads_case = ecg_data
        else:
            # Otherwise, load from file
            ecg_all_leads_case = np.load(case_npy_file_path)
        
        print(ecg_all_leads_case.shape)
        ...
        print(f"FATAL ERROR processing CASE data: {e}"); return None, None
    ...
    try:
        ...
        possible_paths = [
            normal_ref_npy_file_path,  # Try as provided
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "validation01", "validation01.npy"),  # Try from backend dir
            os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "backend", "validation01", "validation01.npy"),  # Try from code dir
            os.path.join("..", "validation", "validation01", "validation01.npy"),  # Try the validation directory
            os.path.join("..", "..", "validation", "validation01", "validation01.npy"),  # Try up two levels
        ]
        
        ecg_all_leads_ref = None
        for path in possible_paths:
            try:
                if os.path.exists(path):
                    ecg_all_leads_ref = np.load(path)
                    print(f"Successfully loaded reference from: {path}")
                    break
            except Exception as e:
                print(f"Failed to load from {path}: {e}")
                continue
            
        if ecg_all_leads_ref is None:
            raise FileNotFoundError(f"Could not find reference ECG file in any of the attempted paths: {possible_paths}")
        ...
        print(f"FATAL ERROR processing REFERENCE data: {e}"); return None, None

    # --- Generate Comparative Plots and store them in a list ---
    print("\n--- Generating Comparative Plots ---")
    
    # Create a list to store figure filenames instead of figure objects
    fig_files = []
    
    # Generate and save the RR interval distribution plot
    rr_dist_fig = plot_rr_interval_distribution_comparative(
    ...
    rr_filename = f"{case_condition_label_short}_rr_distribution_{lead_name_sanitized_case}.png"
    try:
        rr_dist_fig.savefig(rr_filename)
        fig_files.append(rr_filename)
        plt.close(rr_dist_fig)
    except Exception as e:
        print(f"Error saving RR distribution figure: {e}")
    
    # Generate and save the ECG segments plot
    ecg_segments_fig = plot_ecg_segments_comparative(
    ...
    # Save the figure to a temporary file
    ecg_segments_filename = f"{case_condition_label_short}_segments_{lead_name_sanitized_case}.png"
    try:
        ecg_segments_fig.savefig(ecg_segments_filename)
        fig_files.append(ecg_segments_filename)
        plt.close(ecg_segments_fig)
    except Exception as e:
        print(f"Error saving ECG segments figure: {e}")
    ...
    # Return the figure filenames and summary report
    return fig_files, summary_report