...
class ECGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1. Direct path (if it's already a full path)
        ecg_path = ecg_filename if os.path.isfile(ecg_filename) else None
        
        # 2. Join with ecg_dir_path
        if ecg_path is None:
            ecg_path = os.path.join(self.ecg_dir_path, ecg_filename)
            if not os.path.isfile(ecg_path):
                ecg_path = None
        
        # 3. Try to find in subdirectories
        if ecg_path is None:
            # Get the base name without extension
            base_name = os.path.splitext(ecg_filename)[0]
            # Try in a subdirectory with same name as the file
            potential_path = os.path.join(self.ecg_dir_path, base_name, ecg_filename)
            if os.path.isfile(potential_path):
                ecg_path = potential_path
        
        if ecg_path is None:
            raise FileNotFoundError(f"Could not find {ecg_filename} in {self.ecg_dir_path} or its subdirectories")
