...
def main_orchestrator():
    # --- CASE Condition (e.g., RBBB from validation05.npy) ---
    case_npy_file_path = "../validation/validation05/validation05.npy"
    case_condition_label_short = "RBBB" # For file prefix
    case_condition_label_full = "RBBB CASE" # For titles/reports
    ...
    if case_condition_label_short == "RBBB":
        report_lines.append("Key RBBB indicators to check (Compare CASE vs REFERENCE in plots):")
        report_lines.append("  - QRS Duration: Prolonged (>120ms) with characteristic morphology")
        report_lines.append("  - QRS Morphology: RSR' pattern in V1-V2, wide S waves in I, V5-V6")
        report_lines.append("  - ST-T Changes: Secondary repolarization abnormalities")
        report_lines.append("  - Axis: Usually normal")
